...
fit1 <- tmbstan(obj, iter=1000, chains = 2, warmup=200, 
                control = list(max_treedepth = 12, adapt_delta = 0.9), # Stan parameters algorithm
                init='last.par.best', 
...
fit2 <- stan(file='stan_spde.stan', data = data2, 
             iter=1000, chains = 2, warmup=200, control = list(max_treedepth = 5, adapt_delta = 0.6), 
             init=init.fn, pars=names(parameters),

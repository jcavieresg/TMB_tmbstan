...
// ===================================
   ...
   nlp -= dnorm(beta1,     Type(0.0),     Type(5.0), true);   //
...
// Prior in Hyperparameters
  ...
  //projS = A * u;                     // Project S at mesh pts to data pts
    if(model==1)
      //pred  = X*beta + ((A*u) / tau);
      pred  = beta0 + beta1*x1 + ((A*u) / tau);
    if(model==2) 
      //pred  = X*beta +  A*u;
      pred  = beta0 + beta1*x1 + A*u;
    if(model==3) 
      //pred  = X*beta +  A*u;
      pred  = beta0 + beta1*x1 + u(site);
...
// Probability of data conditional on fixed effect values
  for(int i=0; i<n; i++){
    ...
    nll -= dnorm(y(i), pred(i), sigma_e, true);
  }

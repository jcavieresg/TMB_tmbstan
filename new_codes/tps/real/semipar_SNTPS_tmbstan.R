...
library(pacman)
pacman::p_load(geoR, fields, prodlim, TMB, INLA, dplyr, tmbstan, rstan, parallel, mgcv, ggplot2, bayesplot)
...
#             SKEW NORMAL MODEL
#=====================================
obj2 <- MakeADFun(tmb_data, random = c("smoothCoefs"), tmb_par, DLL="semipar_SNTPS_tmbstan")
...
saveRDS(obj, file='fit_tps_TMB.RDS')
...
startTime <- Sys.time()
fit_tps = tmbstan(obj, chains = 3, open_progress = FALSE, 
              init = init.fn, control = list(max_treedepth = 12, adapt_delta = 0.9), 
              iter = 3000, warmup= 700, cores = no_cores, seed=483892929)
endTime <- Sys.time()
timeUsed = endTime - startTime
print(timeUsed)


## 2.55 hours minutes for the complete model!!!


s <- summary(fit_tps, probs = c(0.25, 0.75))
...
# posterior_tps_tmbstan <- as.matrix(fit_tps)
# 
# saveRDS(posterior_tps_tmbstan, file='posterior_tps_tmbstan.RDS')
# saveRDS(fit_tps, file='fit_tps_tmbstan.RDS')
...
pairs(fit_tps, pars = c("logsigma", "logalpha", "lp__"), las = 1) # below the diagonal
pairs(fit_tps, pars = c("logsigma", "logalpha", "loglambda")) # below the diagonal
...
plot(params_cp$iter, params_cp$logalpha, col=c_dark, pch=16, cex=2.2, type = "l",
...
plot(params_cp$iter, running_means_alpha, col=c_dark, pch=16, cex=1.2,  cex.lab=2.0, cex.axis=1.5,
...
mtext("Convergence of the parameters logalpha, logsigma and loglambda", outer=TRUE,  cex=1, line=-0.5)
